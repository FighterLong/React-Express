...
class scienceReader extends Component{
    ...
    width: '25%'
      ...
      render:(text,record,index)=>(
        <span style={{whiteSpace: 'nowrap'}}>
          {
            this.state.permissionInfo.includes(`science:update`) ? <a href="javascript:;" onClick={() => this.editor(text.id)}>编辑</a> : null
          }
          {/* <a href="javascript:;" onClick={() => this.editor(text.id)}>编辑</a> */}
          {
            text.status === 'PUBLISH' && this.state.permissionInfo.includes(`science:publish`)
            ? <span><Divider type="vertical" /><a href="javascript:;" onClick={() => this.publishData([text.id],false)}>取消发布</a></span>
            : null
          }
          {
            text.status === 'UNPUBLISH' && this.state.permissionInfo.includes(`science:publish`)
  ...
  saveSEQ = (data) => {
    ...
    Axios.post(`${URL}/admin/science/update`,data).then(res => {
  ...
  searchKey=(value)=>{
    console.log(value)
    this.setState({getListParams:Object.assign({},this.state.getListParams,{keyword:value})},()=>{
      this.getInfo()
    })
  ...
  // 获取数据
  getInfo=()=>{
    Axios.post(`${URL}/admin/science/retrieveList`,{
        "departmentIds": "",
        "keyword": " ",
        "navbarId": 20,
  ...
  // 删除数据
  delData = (arr) => {
    Axios.post(`${URL}/admin/science/delete?ids=${arr}`).then(res => {
      if(res.data.code === 200) {
  ...
  publishData = (arr,status) => {
    Axios.post(`${URL}/admin/science/publish?ids=${arr}&status=${status}`).then(res => {
      if (res.data.code === 200) {
  ...
  // 新建/编辑  跳转
  editor = (type) => {
    this.props.history.push(`/scienceReaderUpdate/${type}`)
  }
  ...
  componentDidMount() {
    ...
    this.setState({getListParams: {...this.state.getListParams, pageIndex}}, () => {
      this.getInfo()
    })
  }
      ...
      <div className="science-reader">
        ...
        <Form layout="inline">
          ...
          <FormItem style={{float: 'right'}}>
            {
              this.state.permissionInfo.includes(`science:delete`) ?
            ...
            {
              this.state.permissionInfo.includes(`science:publish`) ?
                <span>
            ...
            {
              this.state.permissionInfo.includes(`science:create`) ?
                <span>
